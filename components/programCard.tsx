'use client'
import { MinusCircleIcon, PlusCircleIcon, Trash2Icon, TrashIcon } from "lucide-react"
            ...
            {isOpen && (
                <>
                    ...
                    <AlertDialog open={confirmDelete}>
                        ...
                        <AlertDialogContent>
                            <AlertDialogHeader>
                                <AlertDialogTitle className="text-center">Tem certeza que deseja deletar o diário ?</AlertDialogTitle>
                                    ...
                                    <div className="flex flex-col space-y-2 justify-center">
                                        <div className="flex flex-col justify-center align-middle items-center text-start py-4">
                                            <div className="max-w-[250px] truncate">
                                                <div className="pt-1"><span className="font-semibold ">Cliente: </span>{program.client.name}</div>
                                                <div className="pt-1"><span className="font-semibold">Programa: </span>{program.name}</div>
                                                <div className="pt-1"><span className="font-semibold">Duração: </span> {program.duration} Dias</div>
                                                <div className="pt-1"><span className="font-semibold">Data Início:</span>  {formatDateToDdMmYy(program.start_date)}</div>
                                                <div><span className="font-semibold pt-1">Data Fim:</span>  {formatDateToDdMmYy(program.end_date)}</div>
                                            </div>
                                        </div>
                            ...
                            <AlertDialogFooter>
                                ...
                                </AlertDialogCancel>
                                {
                                    isLoading ? (
                                        ...
                                        :
                                        <>
                                            <AlertDialogAction className="bg-red-600 hover:bg-red-800" onClick={() => handleDeleteProgram(program.id)}>Deletar</AlertDialogAction>
                                        </>
                                }
                            </AlertDialogFooter>
                        </AlertDialogContent>
