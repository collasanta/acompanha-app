...
export const FormButton = ({
  ...
  const getButtonText = () => {
    if (
      checkPoints.find((item) => item.id === day.checkpointId!)?.description ===
      "initial"
    )
      return "Formulário de Avaliação Inicial 📐";
    if (
      checkPoints.find((item) => item.id === day.checkpointId!)?.description ===
      "review"
    )
    ...
    if (
      ...
      return "Formulário de Avaliação Final 📐";
  };
  ...
  return (
    <>
        ...
        <>
          <Button
            className={`text-xs text-white ${
              checkPoints.find((item) => item.id === day.checkpointId!)
                ?.formFilled && "bg-gray-400 hover:bg-gray-400"
            }`}
          >
            {checkPoints.find((item) => item.id === day.checkpointId!)
              ?.formFilled ? (
      ...
      <div
        className={`btn-primary disabled:z-index[-1] flex flex-row space-x-4 py-2 justify-center items-center ${
          checkPoints.find((item) => item.id === day.checkpointId)
            ?.formFilled && "bg-muted"
        }`}
      >
                ...
                <div className="flex space-x-2 items-center align-middle">
                  ...
                  <div>
                    ...
                    <input
                      defaultValue={
                        checkPoints.find(
                          (item) => item.id === day.checkpointId!
                        )?.formUrl!
                      }
                      placeholder={"  colar link aqui"}
                      className="bg-secondary rounded-lg text-cyan-700 pl-3"
                      onBlur={(e) =>
                        setFormsLink(
                          day.checkpointId!,
                          e.target.value,
