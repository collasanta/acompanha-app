                    ...
                    isLoading && index === currentIndex && currentType === 'diet' ?
                        ...
                        <Button
                            ...
                            onClick={async () => {
                                ...
                                setOptimisticDays({ date: day.date, programId: day.programId, diet: !day.diet, exercise: day.exercise, cardio: day.cardio, weight: day.weight, notes: day.notes, checkpointId: day.checkpointId });
            ...
            ${day.exercise ? "bg-[#10B77F] placeholder-white text-white" : day.exercise === null ? day.date.getTime() === currentDate.getTime() ? "bg-muted shadow-lg animate-pulse border  border-black/1" : day.date.getTime() < currentDate.getTime() ? "bg-[#ff6870]" : "bg-muted" : "bg-[#ff6870]"}`}>
                        ...
                        <>
                            <input
                                ...
                                onBlur={async (e) => {
                                    ...
                                    setOptimisticDays({ date: day.date, programId: day.programId, diet: day.diet, cardio: e.target.value === "" ? null : e.target.value, exercise: day.exercise, weight: day.weight, notes: day.notes, checkpointId: day.checkpointId });
                                    setCurrentIndex(index)
                                    setCurrentType("cardio")
                                    setIsLoading(true)
                                    await setCardio(day.date, day.programId, e.target.value, day.cardio ? day.cardio.toString() : null)
                                    setIsLoading(false)
                                }}
                                ...
                                className={`w-[50px] h-[40px] border border-[1px] text-muted-foreground text-[13px] rounded-md align-middle cursor-pointer text-center disabled:text-muted-foreground disabled:opacity-100 
                                 ${day.cardio ? "bg-[#10B77F] text-white font-normal" : day.cardio === null ? day.date.getTime() === currentDate.getTime() ? "bg-muted shadow-lg animate-pulse border-black/1 border" : "bg-muted border" : "bg-[#ff6870] border"}`}>
                            </input>
                        </>
                    :
                        ...
                        <>
                            <input
                                ...
                                onBlur={async (e) => {
                                    ...
                                    setOptimisticDays({ date: day.date, programId: day.programId, diet: day.diet, exercise: day.exercise, cardio: day.cardio, weight: e.target.value === "" ? null : e.target.value, notes: day.notes, checkpointId: day.checkpointId });
                                    setCurrentIndex(index)
                ...
                <div className="w-[50px] max-h-[40px]">
                    {notFuture &&
                        ...
                        <Popover modal={true}>
                            <PopoverTrigger disabled={isLoading} className="min-w-[50px] align-middle flex justify-center items-center">
                                {
