...
export default function DietAutomationEditForm({
  initialAutomation,
  dietPlans,
  ...
  const handleAddRule = () => {
    const lastCalories =
      calorieRules.length > 0
        ? calorieRules[calorieRules.length - 1].calories
        : 900;
    setCalorieRules([
      ...calorieRules,
  ...
  const handleSave = async () => {
    try {
      ...
      toast.error(
        "Falha ao atualizar automação de dieta: " + (error as Error).message
      );
    }
  };
